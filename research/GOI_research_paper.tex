...
A controlled generative validation on three contrasting ontologies-a
familiar Software Services Invoice schema, a custom Job Description
Ontology, and a confidential Pain-Management Clinical Visit Record
...
Description Ontology and 62.2\% on the Pain-Management ontology. The
...
A controlled generative validation on three contrasting ontologies
spanning B2B billing, talent acquisition, and healthcare clinical
documentation demonstrates GOI's structurally-mandated coverage; we
also discuss limitations including LLM consistency, token-window
constraints, the lack of formal OWL axioms, and hallucination risks.
...
evaluate three ontologies of contrasting familiarity: a Software
...
and a Pain-Management Clinical Visit Record Ontology (workers'
...
justification). Full verbatim prompts are included in the
...
non-standard dimensions widen the gap dramatically. All three cases
re-scored under the broadened metric described in
\S\ref{sec:metric}.}
...
{\sloppy\emph{Case~2 (custom).} On the Job Description Ontology
...
\emph{Case~3 (specialised, anonymized).} On the Pain-Management
...
typed graph with YAML/JSON export. Across three contrasting
ontologies in three distinct domains-B2B billing, talent acquisition,
and healthcare clinical documentation-our controlled generative
validation shows GOI-prompted generation covers 95-100\% of the
structural backbone in every case. A generic three-field template
matches GOI on the familiar invoice schema (97.8\%) but collapses to
52.2\% on the custom Job Description Ontology and 62.2\% on the
Pain-Management Clinical Visit Record Ontology-a structural
guarantee generic prompting cannot offer once the document type
leaves the LLM's pretraining prior.
...
synthetic clinically-plausible equivalents), the six LLM-generated
documents used in \S\ref{sec:ontology-as-prompt}, the two prompt
templates (System~A and~B; see Appendix~\ref{app:prompts}), and the
broadened-NCS scoring protocol with per-case detection log. Three
publicly-listed job postings used to induce the JD ontology are at
